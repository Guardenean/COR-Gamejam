...
func _mede_estagio():#calculo: float):
	if Desastres.peso_estagio >= 20:
		estagio = 5
		color = color_5 #Color(0.5, 0, 0.5)  # Roxo
	elif Desastres.peso_estagio >= 15:
		estagio = 4
		color = color_4 #Color(1, 0, 0)  # Vermelho
	elif Desastres.peso_estagio >= 10:
		estagio = 3
		color = color_3 #Color(1, 0.5, 0)  # Laranja
	elif Desastres.peso_estagio >= 5:
		estagio = 2
...
func _on_tempo_jogo_timeout():
	...
	get_tree().change_scene_to_file("res://Cenas/Menu/Final.tscn")
